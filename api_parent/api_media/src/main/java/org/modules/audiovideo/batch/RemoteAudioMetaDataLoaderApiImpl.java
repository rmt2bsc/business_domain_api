...
class RemoteAudioMetaDataLoaderApiImpl extends AbstractTransactionApiImpl implements AvBatchFileProcessorApi {
    ...
    private static final String FTP_USER = "royterrell";
    private static final String FTP_PASSWORD = "hoover";
    ...
    protected RemoteAudioMetaDataLoaderApiImpl(AvBatchImportParameters parms) throws BatchFileProcessException {
        ...
        logger.info("Audio/Video batch processing witll begin at this location: " + parms.getAbsolutePath());
    }
    ...
    public void initConnection(Object parms) throws BatchFileProcessException {
        ...
        this.ftp = FtpFactory.getInstance(FTP_HOST, FTP_PORT, FTP_USER, FTP_PASSWORD, this.parms.getSessionId());

        // Verify that path relativie to the user's home directory is a
        // directory.
        try {
            if (!this.ftp.isDirectory(this.parms.getPath())) {
                this.msg = this.parms.getPath() + " is required to be a directory for Audio Batch process";
    ...
    public void close() {
        try {
            FileUtils.deleteDirectory(new File(this.ftp.getUserSessionWorkArea()));
        } catch (IOException e) {
            logger.error("Failure deleting user's session work area", e);
        }
    }
    ...
    public int processBatch() throws BatchFileProcessException {
        ...
        try {
            ...
            this.processDirectory(null, null);
            return this.totCnt;
    ...
    public Object processDirectory(File mediaResource, Object parent) throws BatchFileProcessException {
        List<String> files = this.ftp.listDirectory(this.parms.getPath(), true);
        if (files == null) {
            return null;
        }

        int actualTotalCount = files.size();
        int ndx = 0;
        try {
            for (String file : files) {
                this.curRemoteFile = file;
                String downloadFilePath = ftp.downloadFile(file);
                File mediaFile = new File(downloadFilePath);
                this.processSingleFile(mediaFile, parent);
                ndx++;
                try {
                    RMT2File.deleteFile(downloadFilePath);
                } catch (Exception e) {
                    logger.error("Failure to delete temporary file, " + downloadFilePath, e);
                }
            }
            boolean matchingTotalCounts = (actualTotalCount == this.totCnt);
            return 1;
...
//        MP3Reader mp3 = AudioVideoDaoFactory.createEntaggedId3Instance(sourceFile);
        ...
        try {
            ...
            // The file path is relative to the user's FTP home directory
            String pathOnly = RMT2File.getFilePathDirectory(this.curRemoteFile);
    ...
    private String getImageFile(String dir) {
        ...
        for (String item : files) {
            if (item.contains(".jpg") || item.contains(".jpeg")) {
                fileName = RMT2File.getFileName(item);
    ...
    private void sendAvBatchReport() throws AvBatchReportException {
        ...
        //
        // // Declare and initialize SMTP ftp and allow the system to discover
        // SMTP
        // // host
        // SmtpApi ftp = SmtpFactory.getSmtpInstance();
        // // Send simple email to its intended destination
        // try {
        // ftp.sendMessage(bean);
        // // Close the service.
        // ftp.close();
        // } catch (Exception e) {
