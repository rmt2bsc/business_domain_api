...
class UserAppRoleApiImpl extends AbstractTransactionApiImpl implements UserAppRoleApi {
    ...
    public List<CategoryDto> getAssignedRoles(CategoryDto criteria) throws UserAppRoleApiException {
        String loginId = criteria.getUsername() == null ? "<user unknown>" : criteria.getUsername();
        RoleDao dao = this.daoFactory.createLdapDao();
        List<CategoryDto> list = null;
        try {
            list = dao.fetchUserAssignedRoles(criteria);
            this.msg = "Total assigned application roles found for user, " + loginId + ": " + list.size();
            logger.info(this.msg);
            return list;
        } catch (Exception e) {
            this.msg = "Unable to fetch assinged Application Roles for user, " + loginId;
    ...
    public List<CategoryDto> getRevokedRoles(CategoryDto criteria) throws UserAppRoleApiException {
        ...
        try {
            ...
            this.msg = "Total revoked application roles found for user, "  + loginId + ": " + list.size();
            logger.info(this.msg);
            return list;
        } catch (Exception e) {
            this.msg = "Unable to fetch revoked application roles for user, " + loginId;
            logger.error(this.msg);
            throw new UserAppRoleApiException(this.msg, e);
        } finally {
            dao.close();
    ...
    public int delete(String userName, List<String> appRoles) throws UserAppRoleApiException {
        ...
        try {
            ...
            this.msg = "Unable to remove application roles for user, " + userName + ": " + list;
            logger.error(this.msg);
            throw new UserAppRoleApiException(this.msg, e);
        } finally {
            dao.close();
